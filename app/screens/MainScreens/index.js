...
import { LiquidGlassView, AndroidGlassBackdrop, glassTint } from "../../components/GlassModules";
...
const CustomTabBar = memo(({ activeRouteName, onTabPress, chatUnreadCount, notificationUnreadCount, onCreatePress }) => {
  ...
  return (
    <View style={{ position: "absolute", bottom: bottomOffset, left: 20, right: 20, flexDirection: "row", alignItems: "center", zIndex: 99 }}>
      {/* Left pill: main tabs */}
      <NavGlassWrapper
        {...(LiquidGlassView ? { variant: "regular", interactive: true, tintColor: glassTint(isDarkMode), borderRadius: 24.5 } : {})}
      ...
      >
        <NavGlassWrapper
          {...(LiquidGlassView ? { variant: "regular", interactive: true, tintColor: glassTint(isDarkMode), borderRadius: 26.5 } : {})}
          style={{
            width: 53, height: 53, borderRadius: 26.5,
            backgroundColor: LiquidGlassView ? "transparent" : surface,
            borderWidth: 1, borderColor: border,
