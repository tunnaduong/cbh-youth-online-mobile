  ...
  return (
    <>
      ...
      >
        ...
        <Animated.Text style={[styles.topTitle, { opacity: headerTitleOpacity, color: theme.text }]}>{t('createPost.title')}</Animated.Text>
        <Animated.View style={{ opacity: headerButtonOpacity }}>
          <LiquidButton size={44} scrollY={scrollY} onPress={handlePost} roundedOnScroll style={styles.publishButton}>
            <Text style={[styles.publishButtonText, { color: theme.primary }]}>{t('createPost.publish')}</Text>
          </LiquidButton>
        </Animated.View>
      </Animated.View>
      ...
      <Animated.ScrollView
        ...
        contentContainerStyle={{ paddingTop: insets.top + 88, paddingBottom: insets.bottom + 24 }}
      ...
      >
        ...
        >
          ...
          <View style={styles.heroCopy}>
            ...
            <Text style={styles.heroSubtitle}>{t('createPost.placeholderContent')}</Text>
          </View>
        </LinearGradient>
        ...
        <View style={[styles.card, { backgroundColor: isDarkMode ? theme.cardBackground : 'rgba(255,255,255,0.96)', borderColor: isDarkMode ? theme.border : 'rgba(15,23,42,0.08)', shadowColor: '#0F172A', shadowOpacity: isDarkMode ? 0.24 : 0.16, shadowRadius: 22, shadowOffset: { width: 0, height: 12 }, elevation: 6 }]}> 
          <View style={styles.profileRow}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Dropdown
                options={isAnonymous ? view.filter((item) => item.value !== 'followers') : view}
          ...
          {selectedImages.length > 0 ? (
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.mediaRow}>
              ...
              <TouchableOpacity onPress={pickImage} style={styles.mediaAddTile}>
                ...
                <Text style={[styles.mediaAddText, { color: theme.primary }]}>{t('createPost.addImage')}</Text>
              </TouchableOpacity>
            </ScrollView>
          ) : (
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  topBar: { position: 'absolute', top: 0, left: 0, right: 0, zIndex: 20, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingBottom: 12, borderBottomWidth: 0, borderBottomColor: 'transparent' },
  ...
  categoryDropdown: { borderRadius: 14, paddingHorizontal: 8, paddingVertical: 6, borderWidth: 1, borderColor: '#E5E7EB' },
